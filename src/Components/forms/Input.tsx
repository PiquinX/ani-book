...
const Input: React.FC<Partial<Props>> = (
  { name, placeholder, type = 'text', describedBy = '', defaultValue, value, onChange, onClick, autoComplete, border = true, style = '' }
) => {
  const id = useId()
  ...
  return (
    <div className="relative">
      <input
        className={`${border ? 'border border-[#333333]' : ''} ${style} w-full h-full px-3 py-2 duration-75 text-md text-white bg-transparent rounded-sm outline-none 
