...
const Input: React.FC<Partial<Props>> = (
  { name, placeholder, type = 'text', describedBy = '', defaultValue = '', border = true, style = '' }
) => {
  const id = useId()
  ...
  return (
        <div className="relative">
            <input
                className={`${border ? 'outline-gray-500' : ''} ${style} w-full h-full px-3 py-2 outline-1 duration-75 text-md text-black  rounded-sm outline-none 
